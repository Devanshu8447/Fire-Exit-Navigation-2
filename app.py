...
def result():
    ...
    if start_room not in room_positions:
        return render_template(
            "result.html", error="Selected start location not found."
        )
    fire_rooms = random.sample(list(room_positions.keys()), 5)
    start_pos = room_positions.get(start_room)
    ...
    for i in range(10):
        ...
        for j in range(10):
            cell = ""
            room = grid[i][j]  # Direct access to the 'grid' variable
            pos = (i, j)
...
def find_escape_path(grid, start_pos, room_positions, fire_rooms):
    ...
    valid_exits = [
        ...
        if room in room_positions and room not in fire_rooms
    ]
